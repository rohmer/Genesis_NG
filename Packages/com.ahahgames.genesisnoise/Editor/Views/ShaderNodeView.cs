...
namespace AhahGames.GenesisNoise.Views
{
    ...
    public class ShaderNodeView : GenesisNodeView
    {
...
#if GENESIS_SHADERGRAPH
            GUIContent shaderGraphContent = EditorGUIUtility.TrTextContentWithIcon("Graph", Resources.Load<Texture2D>("sg_graph_icon@64"));
            menu.AddItem(shaderGraphContent, false, () => SetShader(EditorUtilities.CreateNewShaderGraph(owner.graph, title, dim), shaderCreationUI, shaderField));
#endif
            GUIContent shaderTextContent = EditorGUIUtility.TrTextContentWithIcon("Text", "Shader Icon");
            menu.AddItem(shaderTextContent, false, () => SetShader(EditorUtilities.CreateNewShaderText(owner.graph, title, dim), shaderCreationUI, shaderField));
            menu.ShowAsContext();
        }
