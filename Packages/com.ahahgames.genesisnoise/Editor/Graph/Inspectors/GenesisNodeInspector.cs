...
{
    [CustomEditor(typeof(GenesisNodeInspector))]
    public class GenesisNodeInspectorEditor : NodeInspectorObjectEditor
    {
        ...
        internal GenesisNodeInspector mixtureInspector;

        Dictionary<BaseNode, VisualElement> nodeInspectorCache = new();
        internal List<GenesisNode> nodeWithPreviews = new();
        ...
        {
            mixtureInspector = target as GenesisNodeInspector;
            ...
            previewMaterial = new Material(Shader.Find("Hidden/GenesisInspectorPreview")) { hideFlags = HideFlags.HideAndDontSave };
        ...
        protected override void UpdateNodeInspectorList()
        {
            ...
            {
                var view = CreateGenesisNodeBlock(nodeView, true);
                view.AddToClassList("SelectedNode");
                selectedNodeList.Add(view);

                if (nodeView.nodeTarget is GenesisNode n && n.hasPreview && n.previewTexture != null)
            ...
            {
                var view = CreateGenesisNodeBlock(nodeView, false);
        ...
        VisualElement CreateGenesisNodeBlock(BaseNodeView nodeView, bool selection)
        {
            ...
            unpinButton.RegisterCallback<MouseDownEvent>((e) =>
            {
                ...
                {
                    (nodeView as GenesisNodeView)?.UnpinView();
                }
            });
        ...
        public override void OnPreviewSettings()
        {
            ...
            if (GUILayout.Button(EditorUtilities.fitIcon, EditorStyles.toolbarButton, buttonLayout))
                Fit();
            ...
            if (mixtureInspector.compareEnabled)
            {
                ...
                mixtureInspector.compareMode = (GenesisNodeInspector.CompareMode)EditorGUILayout.Popup((int)mixtureInspector.compareMode, new string[] { " 1  -  Side By Side", " 2  -  Onion Skin", " 3  -  Difference", " 4  -  Swap" }, style, buttonLayout, GUILayout.Width(24));
            ...
            mixtureInspector.compareEnabled = GUILayout.Toggle(mixtureInspector.compareEnabled, EditorUtilities.compareIcon, EditorStyles.toolbarButton, buttonLayout);

            GUILayout.Space(2);

            if (GUILayout.Button(EditorUtilities.settingsIcon, EditorStyles.toolbarButton, buttonLayout))
            {
        ...
        Texture2D GetLockIcon(bool locked) => locked ? EditorUtilities.lockClose : EditorUtilities.lockOpen;
        ...
        public override void OnInteractivePreviewGUI(Rect previewRect, GUIStyle background)
        {
            ...
            if (mixtureInspector.firstLockedPreviewTarget?.previewTexture != null && e.type == EventType.Repaint)
            {
                ...
                GenesisNoiseUtility.SetupDimensionKeyword(previewMaterial, mixtureInspector.firstLockedPreviewTarget.previewTexture.dimension);

                // Set texture property based on the dimension
                GenesisNoiseUtility.SetTextureWithDimension(previewMaterial, "_MainTex0", mixtureInspector.firstLockedPreviewTarget.previewTexture);
                GenesisNoiseUtility.SetTextureWithDimension(previewMaterial, "_MainTex1", mixtureInspector.secondLockedPreviewTarget.previewTexture);
                ...
                previewMaterial.SetVector("_Channels", EditorUtilities.GetChannelsMask(mixtureInspector.channels));
    ...
    [Serializable]
    public class GenesisNodeInspector : NodeInspectorObject
    {
        ...
        internal GenesisNode firstLockedPreviewTarget;
        internal GenesisNode secondLockedPreviewTarget;
