...
namespace AhahGames.GenesisNoise.Graph
{
    public class NodeInspectorSettingsPopupWindow : PopupWindowContent
    {
        ...
        public NodeInspectorSettingsPopupWindow(GenesisNodeInspectorEditor inspector)
        {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (previewTexture != null && previewTexture.previewTexture != null)
            {
                ...
                if (dim == TextureDimension.Tex3D)
                {
                    ...
                    target.texture3DPreviewMode = (GenesisNodeInspector.Texture3DPreviewMode)EditorGUILayout.EnumPopup("Mode", target.texture3DPreviewMode);

                    switch (target.texture3DPreviewMode)
                    {
                        default:
                        case GenesisNodeInspector.Texture3DPreviewMode.Volumetric:
                            ...
                            break;
                        case GenesisNodeInspector.Texture3DPreviewMode.DistanceFieldNormal:
                            ...
                            break;
                        case GenesisNodeInspector.Texture3DPreviewMode.DistanceFieldColor:
                            target.texture3DDistanceFieldOffset = EditorGUILayout.FloatField("SDF Offset", target.texture3DDistanceFieldOffset);
                            target.sdfChannel = (GenesisNodeInspector.SDFChannel)EditorGUILayout.EnumPopup("SDF Offset", target.sdfChannel);
                            target.invertSurface = EditorGUILayout.Toggle("Invert Surface", target.invertSurface);
                            break;
                    }
                }
