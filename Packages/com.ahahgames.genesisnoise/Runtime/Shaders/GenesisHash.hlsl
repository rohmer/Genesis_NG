...
#define HASH_FAST 0
#define HASH_SUPERFAST 1
#define HASH_SINELESS 2
#define HASH_IGN 3
#define HASH_PSEUDO 4
#define HASH_TEA 5
#define TEA_ROUNDS 4
#define HASH_TRIG 6
...
{
    // scale and offset
    v = v * (1.0 / 4320.0) + float2(0.25, 0.0);

    // compute a pseudo-random state via dot + frac
    float state = frac(dot(v * v, float2(3571.0, 3571.0)));

    // square the state, scale, and frac again
...
// Integer hash from Inigo Quilez, converted to HLSL
...
// Integer hash inspired by Inigo Quilez’s iqint3, converted to HLSL.
// Assumes the input is a uint2. If you have float2, reinterpret via asuint().
uint iqint3(uint2 x)
{
    const uint k = 1103515245u;
    ...
    // first mix: shift, xor, multiply
    uint2 q = k * (((x >> 1) ^ yx));

    // final mix: xor lower with shifted upper, multiply
    uint n = k * (q.x ^ (q.y >> 3));
...
void FF(inout float4 v, inout float4 rotate, uint x, uint ac)
{
...
void GG(inout float4 v, inout float4 rotate, uint x, uint ac)
{
...
void HH(inout float4 v, inout float4 rotate, uint x, uint ac)
{
...
void II(inout float4 v, inout float4 rotate, uint x, uint ac)
{
...
float4 md5(float4 u)
{
    float4 digest = float4(A0, B0, C0, D0);
    float4 r, v = digest;
    uint i = 0u;
...
// HLSL translation of PCG-style integer hash functions
// All math operates on uint vectors, reinterpret from float using asuint() if needed
// See: http://jcgt.org/published/0009/03/02/
...
// Tiny Encryption Algorithm (TEA)-inspired integer mixing in HLSL
// Adapted from float2 variant — assumes bitwise-compatible float2 input (use asuint/asfloat)

float2 tea(int rounds, float2 p)
{
    uint2 v = asuint(p);
    uint s = 0u;
    ...
    [loop]
        ...
        {
            s += 0x9E3779B9u;
            v.x += ((v.y << 4u) ^ (v.y + s) ^ (v.y >> 5u));
            v.y += ((v.x << 4u) ^ (v.x + s) ^ (v.x >> 5u));
        }
...
// SuperFastHash – HLSL-converted version
// Original: http://www.azillionmonkeys.com/qed/hash.html
...
/*
float2 trig(float2 p) {
    return float2(float(0xffffff)*frac(43757.5453*sin(dot(float2(p),float2(12.9898,78.233)))));
}
*/
float trig(float2 p)
{
...
// Marsaglia Xorshift RNGs (HLSL version)
// Based on: Journal of Statistical Software, v8n14, 2003

// 128-bit Xorshift (state stored in uint4)
uint xorshift128(inout uint4 v)
{
    // Equivalent to rotating state
    v.w ^= v.w << 11;
    v.w ^= v.w >> 8;

    // Advance state by rotating (wxyz → xyzw)
    uint4 next = uint4(v.w, v.x, v.y, v.z);

    // Mix with shifted value of .y
    next.x ^= next.y;
    next.x ^= next.y >> 19;
...
/// <summary>
/// Different has functions combinedd
/// HashAlgos:
/// 0 = fast 
/// 1 = superfast
/// 2 = Sineless hash
...
/// 6 = Trig
/// </summary>
float hashFunction(in float2 p, int hashAlgo)
{
    if (hashAlgo == HASH_FAST)
    {
        return(fast(p));
    }
    if (hashAlgo == HASH_SUPERFAST)
    {
        return(superfast(p));
    }
    if (hashAlgo == HASH_SINELESS)
    {
        return(hashwithoutsine21(p));
    }
    if (hashAlgo == HASH_IGN)
    {
        return(ign(p));
    }
    if (hashAlgo == HASH_PSEUDO)
    {
        return(pseudo(p));
    }
    if (hashAlgo == HASH_TEA)
    {
        return(tea(TEA_ROUNDS, p));
    }
    if (hashAlgo == HASH_TRIG)
    {
        return(trig(p));
    }
    return(superfast(p));
