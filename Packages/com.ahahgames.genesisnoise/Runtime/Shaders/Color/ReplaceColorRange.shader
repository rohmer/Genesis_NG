Shader "Hidden/Genesis/ReplaceColorRange"
{
    Properties
    {
        ...
        [GenesisColorProperty]_CenterColor("Center Color", Color) = (1,0,0,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 genesis(v2f_customrendertexture i) : SV_Target
            {
                ...
                float3 centerHSV = rgb2hsv(_CenterColor.rgb);
                float3 replaceHSV = rgb2hsv(_ReplaceColor.rgb);

                // Compute normalized distances
                float dh = hueDist(hsv.x, centerHSV.x) / max(_HueRange, 1e-5);
                float ds = abs(hsv.y - centerHSV.y) / max(_SatRange, 1e-5);
                float dv = abs(hsv.z - centerHSV.z) / max(_ValRange, 1e-5);

                // Combine distances (max = OR-like behavior)
