    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 genesis(v2f_customrendertexture i) : SV_Target
            {
                ...
                // --- Apply anisotropy (stretch direction)
                float3 dirA = normalize(float3(dir.x * _Anisotropy, dir.y,dir.z));

                // --- Height samples along anisotropic direction
                float h0 = SampleHeight(uv - dirA * texel * _EmbossWidth, i.direction);
                float h1 = SampleHeight(uv + dirA * texel * _EmbossWidth, i.direction);
