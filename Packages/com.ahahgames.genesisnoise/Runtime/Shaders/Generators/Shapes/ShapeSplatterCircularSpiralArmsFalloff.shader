Shader "Hidden/Genesis/ShapeSplatterCircularSpiralArmsFalloff"
{
    Properties
    {
        ...
        [Tooltip(Number of spiral arms)] _Arms("Arms", Range(1,16)) = 4
        [Tooltip(Instances per arm)] _CountPerArm("Count Per Arm", Range(1,256)) = 64

        [Tooltip(Base radius at start)] _StartRadius("Start Radius", Range(0,1)) = 0.05
        [Tooltip(Radius growth per step)] _RadiusStep("Radius Step", Range(0,1)) = 0.01

        [Tooltip(Base angular step per instance)] _AngleStep("Angle Step", Range(0,6.283)) = 0.25
        ...
        [Tooltip(Falloff strength)] _FalloffStrength("Falloff Strength", Range(0,4)) = 1.0
        [Tooltip(Falloff offset)] _FalloffOffset("Falloff Offset", Range(-1,1)) = 0.0
        [Enum(Linear,0,Smooth,1,Exponential,2)] _FalloffMode("Falloff Mode", int) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ---------------------------------------------------------
            float applyFalloff(float t)
            {
                t = saturate(t + _FalloffOffset);

                if (_FalloffMode == 0)      // Linear
                    return 1.0 - t * _FalloffStrength;

                if (_FalloffMode == 1)      // Smooth
                    return pow(1.0 - smoothstep(0.0, 1.0, t), _FalloffStrength);

                // Exponential
                return exp(-t * 4.0 * _FalloffStrength);
            }

            // ---------------------------------------------------------
            float shapeSplatterCircularSpiralArmsFalloff(float3 uv, float3 dir)
            {
                if (_UseShape == 0)
                    return 0.0;
                ...
                int arms = max((int)_Arms, 1);
                int count = max((int)_CountPerArm, 1);

                [loop]
                for (int arm = 0; arm < arms; arm++)
                {
                    float armOffset = (float)arm * (6.2831853 / arms);
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        // Spiral progression
                        float baseAngle = armOffset + fi * _AngleStep;
                        float baseRadius = _StartRadius + fi * _RadiusStep;

                        // Random jitter
                        float2 rnd = hash21(float2(arm * 131.0 + fi, fi * 31.0));
                        ...
                        // Falloff along arm (0 at start, 1 at end)
                        float t = fi / (float)(count - 1);
                        float f = applyFalloff(t);
            ...
            float4 genesis(v2f_customrendertexture i) : SV_Target
            {
                ...
                float v = shapeSplatterCircularSpiralArmsFalloff(uv, dir);
