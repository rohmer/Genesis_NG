    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float streakNoise(float2 uv)
            {
                ...
                v += (chaos - 0.5) * _StreakChaos;

                v = abs(v);
                v = pow(v, 1.6);

                return v * _StreakStrength;
            }

            // ---------------------------------------------------------
            // Micro drips (tiny vertical streaks)
            // ---------------------------------------------------------
            float microDrips(float2 uv)
            {
                float2 p = uv * float2(40, 4);
                float n = hash11(floor(p.x) + floor(p.y) * 19.0);
                return pow(n, 4.0) * _MicroDrips;
            }
