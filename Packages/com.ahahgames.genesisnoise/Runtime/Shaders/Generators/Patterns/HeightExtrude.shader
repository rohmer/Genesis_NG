Shader "Hidden/Genesis/HeightExtrude"
{
    Properties
    {
        ...
        [Enum(Disabled,0,Enabled,1)] _UseHeight("Use Height", int) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Height Extrude core
            // ---------------------------------------------------------
            float heightExtrude(float3 uv, float3 dir)
            {
                if (_UseHeight == 0)
                    return 0.0;
                ...
                // Directional max sampling
                [loop]
                for (int i = 0; i < (int)_Steps; i++)
                {
                    float v = SAMPLE_X(_Height, float3(p, uv.z), dir).r;
                    h = max(h, v);
                    p += delta;
                }

                return pow(h, _Contrast);
            }
