Shader "Hidden/Genesis/GrungeDirt2"
{
    Properties
    {
        ...
        _MidBlotchRadius("Mid Blotch Radius", Range(0.1,3.0)) = 1.1
        _MidBlotchSoftness("Mid Blotch Softness", Range(0.5,10.0)) = 2.5
        ...
        _MicroSpeckScale("Micro Speck Scale", Range(20,120)) = 50.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ---------------------------------------------------------
            // Mid-scale blotches (between Dirt 1 and Dirt Fine)
            // ---------------------------------------------------------
            float midBlotch(float2 uv)
            {
                ...
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        float g = exp(-_MidBlotchSoftness * (d / _MidBlotchRadius) * (d / _MidBlotchRadius));
            ...
            float4 mixture(v2f_customrendertexture i) : SV_Target
            {
                ...
                float mb = midBlotch(uv);
                float fd = fineDust(uv);
                float ms = microSpecks(uv);
                float oc = softOcclusion(uv);
                float br = breakup(uv);
