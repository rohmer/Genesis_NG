...
{	
	Properties
	 {
        [Tooltip(Use source as color input or generate from selected colors)][Enum(Input,0,Generated,1)]_colorSource("Color Source",int)=0
	...
	SubShader
	{
		...
		Pass
		{
            ...
            float _Seed;
            int _colorSource;
            float4 _colorCurve1,_colorCurve2;

            // --------------------------------------------
            // Helpers: cheap noise + fbm
            ...
            float2 WarpUV(float2 uv, float time)
            {
                ...
                float s = sin((uv.xyx * float3(1, 1, 0)).x * _WarpFreq + time) *
            ...
            float3 SampleChromatic(float2 uv, float2 center, float shift, float3 direction)
            {
                ...
                if(_colorSource==0)             // This is a modifier
                {
                ...
                // Get chromatic shift as 0-1
                float r = GetColor(uvR,direction).r;
                float g = GetColor(uvG,direction).g;
                float b = GetColor(uvB,direction).b;
                float sVal=(r+g+b)/3;        
                float3 c=lerp(_colorCurve1,_colorCurve2,sVal);
                return c;
            ...
            { 
                ...
                // Time (Unity: _Time.y is seconds/20; _Time.x is time/20; _Time.z is seconds)
                float t = _Seed * _WarpSpeed;

                // Nonlinear radial warp (optional; set _BarrelK=0 to disable)
                ...
                // Optional subtle contrast curve
                col = saturate(col);
