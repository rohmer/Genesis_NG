...
{
    [Serializable]
    public class GenesisNoiseSettings
    {
        ...
        public int depth;
        public POTSize potSize = POTSize._2048;
        ...
        [NonSerialized]
        internal GenesisNoiseSettings resolvedSettings;

        public static GenesisNoiseSettings defaultValue
        {
            get
            {
                GenesisNoiseSettings set = new()
                {
                    widthScale = 1.0f,
                    heightScale = 1.0f,
                    depthScale = 1.0f,
        ...
        public GenesisNoiseSettings()
        {
            resolvedSettings = this;
        }

        public void ResolveAndUpdate(GenesisNode node)
        {
            this.node = node;
            ...
            if (resolvedSettings == this || resolvedSettings == null)
                resolvedSettings = new GenesisNoiseSettings();

            resolvedSettings.dimension = ResolveTextureDimension(node, graph);
            resolvedSettings.width = Mathf.Clamp(ResolveWidth(node, graph), 1, maxTextureResolution);
            resolvedSettings.height = Mathf.Clamp(ResolveHeight(node, graph), 1, maxTextureResolution);
            resolvedSettings.depth = Mathf.Clamp(ResolveDepth(node, graph), 1, maxTextureResolution);
            ...
            int ResolveWidth(GenesisNode node, GenesisGraph graph)
            {
            ...
            int ResolveHeight(GenesisNode node, GenesisGraph graph)
            {
                switch (node.settings.sizeMode)
                {
                    default:
            ...
            int ResolveDepth(GenesisNode node, GenesisGraph graph)
            {
            ...
            OutputPrecision ResolveOutputPrecision(GenesisNode node, GenesisGraph graph)
            {
            ...
            OutputChannel ResolveOutputChannels(GenesisNode node, GenesisGraph graph)
            {
            ...
            OutputDimension ResolveTextureDimension(GenesisNode node, GenesisGraph graph)
            {
            ...
            OutputWrapMode ResolveWrapMode(GenesisNode node, GenesisGraph graph)
            {
                // if this function is called from the output node and the dimension is default, then we set it to a default value
            ...
            OutputFilterMode ResolveFilterMode(GenesisNode node, GenesisGraph graph)
            {
                // if this function is called from the output node and the dimension is default, then we set it to a default value
        ...
        public bool IsHDR(GenesisGraph graph)
        {
        ...
        public GraphicsFormat GetGraphicsFormat(GenesisGraph graph)
            => ConvertToGraphicsFormat(GetResolvedChannels(graph), GetResolvedPrecision(graph));

        public int GetResolvedWidth(GenesisGraph graph) => resolvedSettings.width;
        public int GetResolvedHeight(GenesisGraph graph) => resolvedSettings.height;
        public int GetResolvedDepth(GenesisGraph graph) => resolvedSettings.depth;
        public OutputPrecision GetResolvedPrecision(GenesisGraph graph) => resolvedSettings.outputPrecision;
        public OutputChannel GetResolvedChannels(GenesisGraph graph) => resolvedSettings.outputChannels;
        public TextureDimension GetResolvedTextureDimension(GenesisGraph graph) => (TextureDimension)resolvedSettings.dimension;
        public TextureWrapMode GetResolvedWrapMode(GenesisGraph graph) => (TextureWrapMode)resolvedSettings.wrapMode;
        public FilterMode GetResolvedFilterMode(GenesisGraph graph) => (FilterMode)resolvedSettings.filterMode;

        public bool NeedsUpdate(GenesisGraph graph, Texture t, bool checkFormat = true)
        {
        ...
        public GenesisNoiseSettings Clone()
        {
