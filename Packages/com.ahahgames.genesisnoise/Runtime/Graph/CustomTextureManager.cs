...
namespace AhahGames.GenesisNoise.Graph
{
    ...
    public static class CustomTextureManager
    {
        ...
        static List<CustomRenderTexture> sortedCustomRenderTextures = new();

        static HashSet<CustomRenderTexture> needsInitialization = new();
        static Dictionary<CustomRenderTexture, int> needsUpdate = new();

        static Dictionary<CustomRenderTexture, int> computeOrder = new();
        static Dictionary<CustomRenderTexture, CustomSampler> customRenderTextureSamplers = new();
        static Dictionary<CustomRenderTexture, double> customRenderTextureLastUpdateTime = new();
        ...
        if (GraphicsSettings.renderPipelineAsset == null)
        {
            ...
            if (Resources.FindObjectsOfTypeAll<BuiltinRealtimeGenesisUpdater>().Length == 0)
            {
                var mixtureUpdateObject = new GameObject("Realtime Genesis Updater");
                GameObject.DontDestroyOnLoad(mixtureUpdateObject);
                var builtinUpdater = mixtureUpdateObject.AddComponent<BuiltinRealtimeGenesisUpdater>();
        ...
        public static Dictionary<CustomRenderTexture, CustomTextureExecInfo> crtExecInfo = new();
        ...
        public static void UpdateCustomRenderTexture(CommandBuffer cmd, CustomRenderTexture crt, int updateCount, int mipLevel = 0, MaterialPropertyBlock block = null)
        {
            ...
            for (int i = 0; i < updateCount; i++)
            {
                ...
                List<CustomRenderTextureUpdateZone> updateZones = new();
