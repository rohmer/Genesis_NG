...
namespace AhahGames.GenesisNoise.Nodes
{
    ...
    public abstract class GenesisNode : BaseNode
    {
        ...
        public Texture2D GetThemeIcon()
        {
            return ThemeManager.GetInstance().GetNodeIcon(NodeGroup, name);
        }

        public Color GetBackgroundColor()
        {
            return ThemeManager.GetInstance().GetBackgroundColor(NodeGroup, name);
        }
        ...
        protected virtual GenesisNoiseSettings defaultSettings => GenesisNoiseSettings.defaultValue;
        [SerializeField]
        public VariableStorage nodeVariables = new();
        public virtual float nodeWidth => GenesisNoiseUtility.defaultNodeWidth;
        public virtual Texture previewTexture => null;
        public virtual bool hasSettings => true;
        public virtual bool hasPreview => true;

        public virtual bool isOutputNode => false;


        public virtual List<OutputDimension> supportedDimensions => new() {
        ...
        protected Dictionary<string, Material> temporaryMaterials = new();
        ...
        [NonSerialized]
        internal GenesisNode parentSettingsNode;
        [NonSerialized]
        internal GenesisNode childSettingsNode;

        public virtual string NodeGroup => "Default";
        ...
        protected GenesisNoiseSettings GetCubeOnlyRTSettings(GenesisNoiseSettings defaultSettings)
        {
        ...
        internal virtual float processingTime
        {
            get
            {
                if (recorder != null)
                    return recorder.gpuElapsedNanoseconds / 10000000.0f;
                return 0;
            }
        }
        ...
        protected GenesisNoiseSettings Get3DOnlyRTSettings(GenesisNoiseSettings defaultSettings)
        {
            var rtSettings = defaultSettings;
        ...
        {
            bool settings = n is GenesisNode m && m.hasSettings;
        ...
        {
            // Update nodes used to infer settings values
            parentSettingsNode = GetInputNodes().FirstOrDefault(n => IsNodeUsingSettings(n)) as GenesisNode;
            childSettingsNode = GetOutputNodes().FirstOrDefault(n => IsNodeUsingSettings(n)) as GenesisNode;
            ...
            GraphicsFormat overrideGraphicsFormat = GraphicsFormat.None, bool hideAsset = true)
        {
            ...
            if (target == null)
            {
                ...
                target.Create();
                target.material = GenesisNoiseUtility.dummyCustomRenderTextureMaterial;
        ...
        static Regex tooltipRegex = new(@"Tooltip\((.*)\)");
        ...
        protected IEnumerable<PortData> GetMaterialPortDatas(Material material)
        {
            ...
            for (int i = 0; i < material.shader.GetPropertyCount(); i++)
            {
                ...
                {
                    bool is2D = displayName.EndsWith(GenesisNoiseUtility.texture2DPrefix);
                    bool is3D = displayName.EndsWith(GenesisNoiseUtility.texture3DPrefix);
                    bool isCube = displayName.EndsWith(GenesisNoiseUtility.textureCubePrefix);
        ...
        {
            return GenesisNoiseUtility.GetAllowedDimentions(name).Contains(dim);
        }
        ...
        protected void AssignMaterialPropertiesFromEdges(List<SerializableEdge> edges, Material material)
        {
            ...
            foreach (var edge in edges)
            {
                ...
                {
                    case ShaderPropertyType.Color:
                        material.SetColor(propName, GenesisTypeConverter.ConvertObjectToColor(edge.passThroughBuffer));
                        break;
                    case ShaderPropertyType.Texture:
                    ...
                    case ShaderPropertyType.Range:
                        ...
                        break;
                    case ShaderPropertyType.Vector:
                        material.SetVector(propName, GenesisTypeConverter.ConvertObjectToVector4(edge.passThroughBuffer));
                        break;
                }
            }
        }
        ...
        // For all nodes that inherit GenesisNode, we provide a function that automatically changes the port type depending on the texture dimension:
