...
")]
    [System.Serializable, NodeMenuItem("Generators/Points/Brownian Noise Points")]
    public class BrownianNoisePointsNode : GenesisNode
    {
        ...
        [Input, SerializeField, Range(0.1f, 8f)]
        public float baseFrequency = 1.25f;

        [Input, SerializeField, Range(1, 10)]
        public int octaves = 6;

        [Input, SerializeField, Range(0.05f, 0.85f)]
        public float amplitudeFalloff = 0.35f;

        [Input, SerializeField, Range(0.1f, 4f)]
        public float densityPower = 2f;
        ...
        public override string name => "Brownian Noise Points";
        ...
        bool TryBuildCumulativeWeights(int resolution, out float totalWeight)
        {
            ...
            float safeFalloff = Mathf.Clamp(amplitudeFalloff, 0.05f, 0.85f);
            ...
            for (int y = 0; y < resolution; y++)
            {
                ...
                for (int x = 0; x < resolution; x++)
                {
                    ...
                    float density = BrownianNoise01(u, v, safeBaseFrequency, safeOctaves, safeFalloff, seed);
        ...
        static float BrownianNoise01(float u, float v, float baseFrequency, int octaves, float amplitudeFalloff, int seed)
        {
            ...
            {
                float octaveWeight = amplitude / Mathf.Max(1f, frequency / baseFrequency);
                sum += ValueNoise(u * frequency, v * frequency, seed + octave * 1619) * octaveWeight;
                amplitudeSum += octaveWeight;
                frequency *= 2f;
                amplitude *= amplitudeFalloff;
            }

            return amplitudeSum > epsilon ? sum / amplitudeSum : 0f;
        }
        ...
        bool NeedsRebuild(int width, int height)
        {
            ...
            if (Mathf.Abs(baseFrequency - lastBaseFrequency) > epsilon
                || Mathf.Abs(amplitudeFalloff - lastAmplitudeFalloff) > epsilon
        ...
        void CacheSettings(int width, int height)
        {
            ...
            lastAmplitudeFalloff = amplitudeFalloff;
