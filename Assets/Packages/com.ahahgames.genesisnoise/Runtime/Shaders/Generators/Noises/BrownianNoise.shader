Shader "Hidden/Genesis/BrownianNoise"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float BrownianHash(float3 cell, float period, int seed)
			{
			...
			float ValueNoise(float3 coordinate, float period, int seed)
			{
				...
				float c000 = BrownianHash(cell + float3(0, 0, 0), period, seed);
				float c100 = BrownianHash(cell + float3(1, 0, 0), period, seed);
				float c010 = BrownianHash(cell + float3(0, 1, 0), period, seed);
				float c110 = BrownianHash(cell + float3(1, 1, 0), period, seed);
				float c001 = BrownianHash(cell + float3(0, 0, 1), period, seed);
				float c101 = BrownianHash(cell + float3(1, 0, 1), period, seed);
				float c011 = BrownianHash(cell + float3(0, 1, 1), period, seed);
				float c111 = BrownianHash(cell + float3(1, 1, 1), period, seed);
			...
			float BrownianNoise01(float3 uvs, int seed)
			{
				...
				float falloff = clamp(_Falloff, 0.01, 0.95);
				float amplitude = 1.0;
			...
			float GenerateNoise(v2f_customrendertexture i, int seed)
			{
				...
				float noise = BrownianNoise01(uvs, seed);
