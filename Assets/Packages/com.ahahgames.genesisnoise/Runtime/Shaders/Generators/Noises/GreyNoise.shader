Shader "Hidden/Genesis/GreyNoise"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float GreyHash(float3 cell, float period, int seed)
			{
			...
			float ValueNoise(float3 coordinate, float period, int seed)
			{
				...
				float c000 = GreyHash(cell + float3(0, 0, 0), period, seed);
				float c100 = GreyHash(cell + float3(1, 0, 0), period, seed);
				float c010 = GreyHash(cell + float3(0, 1, 0), period, seed);
				float c110 = GreyHash(cell + float3(1, 1, 0), period, seed);
				float c001 = GreyHash(cell + float3(0, 0, 1), period, seed);
				float c101 = GreyHash(cell + float3(1, 0, 1), period, seed);
				float c011 = GreyHash(cell + float3(0, 1, 1), period, seed);
				float c111 = GreyHash(cell + float3(1, 1, 1), period, seed);
			...
			float GreyNoise01(float3 uvs, int seed)
			{
				...
				float highFrequencyLift = clamp(_HighFrequencyLift, 0.0, 4.0);
				float total = 0.0;
				float weightSum = 0.0;
				...
				for (int octave = 0; octave < 12; octave++)
				{
					...
					float octavePosition = octaveCount > 1 ? octave / (float)(octaveCount - 1) : 0.0;
					float weight = lerp(1.0, 1.0 + highFrequencyLift, octavePosition);
					float octaveFrequency = max(frequency, 1.0);
					total += ValueNoise(uvs * octaveFrequency, octaveFrequency, seed + octave * 1223) * weight;
					weightSum += weight;
			...
			float GenerateNoise(v2f_customrendertexture i, int seed)
			{
				...
				float noise = GreyNoise01(uvs, seed);
