Shader "Hidden/Genesis/MandelbrotOrbitTrap"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 MandelbrotColor(float2 p)
            {
                float2 c = _Center.xy + p * _Scale;
                ...
                float t = smooth / maxIt;

                // Base palette
                float3 baseCol = 0.5 + 0.5 * cos(_ColorOffset + smooth * _ColorFrequency + float3(0.0, 2.0, 4.0));
