Shader "Hidden/Genesis/Grunge008"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ---------------------------------------------------------
            // Crack generator (sharp cellular breakup)
            // ---------------------------------------------------------
            float cracks(float2 uv)
            {
                ...
                for (int y = -1; y <= 1; y++)
                {
                    ...
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        // Sharper than blotches — crack-like
                        float g = exp(-_CrackSharpness * (d / _CrackRadius));
            ...
            // ---------------------------------------------------------
            // Plate breakup (fractured islands)
            // ---------------------------------------------------------
            float plates(float2 uv)
            {
                float n = fbm(uv * _PlateNoiseScale);
                return n * _PlateNoise;
            }
