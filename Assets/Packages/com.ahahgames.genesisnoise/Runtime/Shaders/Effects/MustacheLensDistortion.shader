Shader "Hidden/Genesis/MustacheLensDistortion"
{
    Properties
    {
        ...
        [Tooltip(Outward barrel bend near the center)] _CenterBarrel("Center Barrel", Range(0,1)) = 0.36
        [Tooltip(Inward pincushion correction near the edges)] _EdgePincushion("Edge Pincushion", Range(0,1)) = 0.34
        [Tooltip(Radius where barrel transitions into pincushion)] _Transition("Transition", Range(0.05,1.5)) = 0.58
        ...
        [Tooltip(Edge fade for stretched borders)] _EdgeFade("Edge Fade", Range(0,1)) = 0.22
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 DistortUV(float2 uv)
            {
                ...
                float edgeBlend = smoothstep(_Transition * 0.55, _Transition, r);
                float barrel = _CenterBarrel * r2;
                float pincushion = _EdgePincushion * r2 * r2;
                float factor = 1.0 + barrel * (1.0 - edgeBlend) - pincushion * edgeBlend;
