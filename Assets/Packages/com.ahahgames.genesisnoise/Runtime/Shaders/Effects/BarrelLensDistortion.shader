Shader "Hidden/Genesis/BarrelLensDistortion"
{
    Properties
    {
        ...
        [Tooltip(Barrel distortion amount, negative values create pincushion distortion)] _Distortion("Distortion", Range(-1,1)) = 0.35
        [Tooltip(Higher order distortion for stronger edge bend)] _CubicDistortion("Cubic Distortion", Range(-1,1)) = 0.12
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float2 DistortUV(float2 uv, float amount, float cubicAmount)
            {
                float2 p = (uv - _Center) / max(_Zoom, 0.0001);
                float r2 = dot(p, p);
                float factor = 1.0 + amount * r2 + cubicAmount * r2 * r2;
